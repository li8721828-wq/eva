...
export function CostCenter() {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-auto bg-white">
      <header className="flex flex-wrap items-start justify-between gap-4 border-b border-zinc-200 px-7 py-5">
      ...
      <main className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-7 py-6">
        <div className="flex flex-wrap items-end gap-3 border-b border-zinc-100 pb-5">
...
function Metric({ icon: Icon, label, value, detail }: { icon: React.ComponentType<{ className?: string }>; label: string; value: string; detail: string }) {
  return <div className="border border-zinc-200 bg-white p-4"><div className="flex items-center gap-2 text-xs font-medium text-zinc-500"><Icon className="h-3.5 w-3.5 text-violet-500" />{label}</div><p className="mt-3 text-2xl font-semibold text-zinc-900">{value}</p><p className="mt-1 truncate text-xs text-zinc-400">{detail}</p></div>
}
